...
export default function PaymentDashboard() {
  ...
  const typeBreakdown = useMemo(() => {
    const rental = payments.filter((p) => (p.status === "completed" || p.status === "captured") && (p.payment_type === "rental" || p.payment_type === "P" || p.payment_type === "PAC")).reduce((s, p) => s + p.amount, 0);
    const deposit = payments.filter((p) => (p.status === "completed" || p.status === "captured") && (p.payment_type === "deposit" || p.payment_type === "PA")).reduce((s, p) => s + p.amount, 0);
