 ...
 * or owns them.
 */
export async function filterTicketsByBranch<T extends Record<string, any> & { id: string }>(tickets: T[], locationId: string | null, viewerUserId?: string | null): Promise<T[]> {
...
export function useCreateCustomerTicketV2() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      console.error("Failed to create ticket:", error);
      toast.error(error?.message ? `Failed to create ticket: ${error.message}` : "Failed to create ticket");
    },
  });
}
