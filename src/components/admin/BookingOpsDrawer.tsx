...
import { ModifyRentalPanel } from "./ops/ModifyRentalPanel";
...
export function BookingOpsDrawer({ bookingId, open, onClose }: BookingOpsDrawerProps) {
  ...
  const handleReopenRental = () => {
    if (!bookingId) return;
    updateStatus.mutate(
      { bookingId, newStatus: "active" as BookingStatus, notes: "Rental reopened by staff to re-enter correct return details", reopen: true, skipNotifications: true },
      {
        onSuccess: () => {
